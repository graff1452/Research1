...
print("🚀 BERT CoLA Full Fine-tuning (with checkpoint-averaged power/VRAM)")
print("=" * 60)

# ------------ Config ------------
MODEL_NAME   = "bert-base-uncased"
OUTPUT_DIR   = "./bert-cola-fullft"
MAX_LENGTH   = 128
EPOCHS       = 5
BATCH_SIZE   = 32
GRAD_ACCUM   = 2
...
SAVE_STEPS   = 200
GPU_INDEX    = 0
OPTIM        = "adamw_torch"  # BERT: keep simple/stable. (paged_adamw_8bit requires bitsandbytes)
...
# ------------ Metrics (CoLA uses Matthews Correlation) ------------
mcc = evaluate.load("matthews_correlation")
def compute_metrics(eval_pred):
    logits, labels = eval_pred
    preds = np.argmax(logits, axis=1)
    return mcc.compute(predictions=preds, references=labels)
...
args_kwargs = dict(
    ...
    gradient_checkpointing=False,  # usually unnecessary for BERT; set True only if you need memory savings
    optim=OPTIM,
)
...
# ------------ Train ------------
print("\n🚀 Starting FULL fine-tuning (CoLA)…")
t0 = time.time()
...
print("\n🏁 FULL FINE-TUNING SUMMARY (CoLA)")
print(f" • Zero-shot MCC: {zs_mcc:.4f}")
print(f" • Fine-tuned MCC: {ft_mcc:.4f}  (Δ {impr:.4f})")
