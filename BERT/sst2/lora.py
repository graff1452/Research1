...
print("🚀 BERT SST-2 LoRA Fine-tuning (with checkpoint-averaged power/VRAM)")
print("=" * 70)

# --------------------------- Config ---------------------------
MODEL_NAME   = "bert-base-uncased"
OUTPUT_DIR   = "./bert-sst2-lora"
BATCH_SIZE   = 32
LEARNING_RATE= 1e-5
EPOCHS       = 5
MAX_LENGTH   = 128
...
# BERT has pad token already; no eos/pad remapping needed.
...
# --------------------------- LoRA config ---------------------------
print("\n🔧 Configuring LoRA...")
# For BERT attention, target modules are typically: query, key, value, and output.dense
lora_config = LoraConfig(
    task_type=TaskType.SEQ_CLS,
    r=16,
    lora_alpha=32,
    lora_dropout=0.1,
    target_modules=["query", "key", "value", "dense"],  # includes attention output dense + other dense; see note below
    bias="none",
)

# NOTE: If you want to restrict LoRA strictly to attention projections only, use:
# target_modules=["query", "key", "value"]
# Some HF BERT modules also expose "attention.output.dense" as "dense"; leaving "dense"
# will also LoRA-wrap intermediate/FFN dense layers depending on PEFT version's matching.
