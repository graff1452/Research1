...
def tokenize(batch):
    texts = [
        f"Premise: {p}\nHypothesis: {h}\nTask: entailment/neutral/contradiction. Answer:"
...
class Mamba2SequenceClassificationModel(torch.nn.Module):
    def __init__(self, backbone: torch.nn.Module, num_labels: int = 3):
        super().__init__()
...
def make_ft():
    ...
    model = Mamba2SequenceClassificationModel(backbone, 3).to(device)
...
def make_lora(r=8, alpha=16, dropout=0.1):
    ...
    cfg = LoraConfig(r=r, lora_alpha=alpha, target_modules=targets, lora_dropout=dropout,
...
def make_qlora():
    ...
    targets = mamba_safe_lora_targets(backbone)
    cfg = LoraConfig(r=8, lora_alpha=16, target_modules=targets, lora_dropout=0.1,
                     bias="none", task_type="SEQ_CLS")
    backbone = get_peft_model(backbone, cfg)
    model = Mamba2SequenceClassificationModel(backbone, 3)
    ensure_head_initialized(model)
    return model
...
def run_experiment(method_name, make_model_fn, lr, epochs, optim_name="adamw_torch"):
    ...
    try:
        ...
        val_m = dataset["validation_matched"].remove_columns([c for c in dataset["validation_matched"].column_names if c not in keep_cols])
        ...
        m_matched = trainer.evaluate(eval_dataset=val_m)
        m_mism    = trainer.evaluate(eval_dataset=val_mm)
