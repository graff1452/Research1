...
def tokenize(batch):
    texts = [
        f"Question 1: {q1}\nQuestion 2: {q2}\nTask: Are these duplicates? Answer:"
...
# -----------------------------
# Mamba2 seq-cls wrapper (lazy head, stable)
# -----------------------------
...
def make_qlora():
    ...
    # checkpointing off reduces shape-path crashes in 4-bit + custom backbones
...
def run_experiment(method_name: str, make_model_fn, lr: float, epochs: int, optim_name: str = "adamw_torch"):
    ...
    try:
        ...
        args = TrainingArguments(
            output_dir=f"./runs/qqp/{method_name.replace(' ','_')}",
            eval_strategy="epoch",
            save_strategy="no",
            learning_rate=lr,
...
out_csv = "mamba2_370m_qqp_benchmark_results.csv"
