...
print("🚀 TinyLlama QNLI Adapter Fine-tuning (with checkpoint-averaged power/VRAM)")
print("=" * 72)

# ---- Config ----
MODEL_NAME   = "TinyLlama/TinyLlama-1.1B-Chat-v1.0"
TASK_NAME    = "qnli"
OUTPUT_DIR   = "./tinyllama-qnli-adapters"
...
print("🔧 Tokenizing…")
ds = raw.map(tokenize, batched=True, remove_columns=["question", "sentence", "idx"])
...
# GLUE/QNLI labels: 0 = ENTAILMENT, 1 = NOT_ENTAILMENT
...
args_kwargs = dict(
    ...
    metric_for_best_model="eval_accuracy",   # QNLI uses accuracy
...
# ---- Train ----
print("\n🚀 Starting Adapter fine-tuning (QNLI)…")
t0 = time.time()
...
# ---- Quick predictions (QNLI: question, sentence) ----
print("\n🎯 Sample predictions (QNLI-style)")
def predict(question: str, sentence: str):
    ...
    with torch.no_grad():
        ...
        label = "ENTAILMENT" if cls == 0 else "NOT_ENTAILMENT"
        conf = probs[cls].item()
    return label, conf
...
print("\n🏁 ADAPTER FINE-TUNING SUMMARY (QNLI)")
