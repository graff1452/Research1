...
print("🚀 TinyLlama QNLI BitFit Fine-tuning (with checkpoint-averaged power/VRAM)")
print("=" * 60)
...
OUTPUT_DIR    = "./tinyllama-qnli-bitfit"   # <-- changed
BATCH_SIZE    = 32
LEARNING_RATE = 1e-5
EPOCHS        = 3
MAX_LENGTH    = 128
...
dataset = load_dataset("glue", "qnli")      # <-- changed
...
model = AutoModelForSequenceClassification.from_pretrained(
    MODEL_NAME,
    num_labels=2,
    # GLUE/QNLI label ids: 0 = not_entailment, 1 = entailment
    id2label={0: "NOT_ENTAILMENT", 1: "ENTAILMENT"},   # <-- changed
    label2id={"NOT_ENTAILMENT": 0, "ENTAILMENT": 1},   # <-- changed
...
def tokenize_function(examples):
    enc = tokenizer(
        examples["question"],              # <-- changed
        examples["sentence"],              # <-- changed
        truncation=True,
        padding=False,
...
tokenized = dataset.map(
    tokenize_function,
    batched=True,
    remove_columns=["question", "sentence", "idx"],  # <-- changed
    desc="Tokenizing",
)
...
# ---------------------------
acc_metric = evaluate.load("accuracy")   # <-- changed
...
zs_acc = pre_eval.get("eval_accuracy", 0.0)   # <-- changed
...
ft_acc = post_eval.get("eval_accuracy", 0.0)  # <-- changed
...
results = {
    "method": "BitFit",
    "task": "GLUE-QNLI",                  # <-- changed
    "model_name": MODEL_NAME,
    "batch_size": BATCH_SIZE,
    "learning_rate": LEARNING_RATE,
    "epochs": EPOCHS,
    "max_length": MAX_LENGTH,
    "zero_shot_accuracy": zs_acc,         # <-- changed field names
