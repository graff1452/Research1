# tinyllama_qqp_sequential_csv.py
# Creates 6 per-method CSVs (and an optional rollup) and runs methods sequentially.
# If a method CSV already exists, it can skip that method (resume-friendly).
...
# Adapters may not support TinyLlama in your env. If it fails, row will be FAILED.
try:
    import adapters
...
metric = evaluate.load("glue", "qqp")  # accuracy + f1
...
MAX_LEN = 128  # QQP questions can be longer

def tokenize(batch):
    return tokenizer(
        batch["question1"], batch["question2"],
        truncation=True,
        max_length=MAX_LEN,
    )
...
def build_base_model(load_4bit: bool = False):
    ...
    # critical for Llama-family seq-cls batching
...
def make_adapters():
    ...
    adapters.init(model)
    model.add_adapter("qqp", config="seq_bn")
...
# -----------------------------
# Per-method CSV writing (sequential)
# -----------------------------
CSV_DIR = Path("./runs/qqp/csv")
CSV_DIR.mkdir(parents=True, exist_ok=True)

ROLLUP_PATH = CSV_DIR / "_ALL.csv"   # cumulative results
WRITE_ROLLUP = True                 # set False if you only want per-method CSVs
...
def write_single_row_csv(row: dict, path: Path) -> None:
    df = pd.DataFrame([{k: row.get(k, None) for k in CSV_COLUMNS}])
    path.parent.mkdir(parents=True, exist_ok=True)
    df.to_csv(path, index=False)

def append_rollup_csv(row: dict, path: Path) -> None:
    df_row = pd.DataFrame([{k: row.get(k, None) for k in CSV_COLUMNS}])
    if path.exists():
...
def run_experiment(method_name: str, make_model_fn, lr: float, epochs: int, optim_name: str = "adamw_torch"):
    ...
    try:
        ...
        # QQP is longer/heavier; keep your settings
        per_device_bs = 16
        grad_accum = 1

        args = TrainingArguments(
            output_dir=f"./runs/qqp/{safe_method_filename(method_name)}",
            eval_strategy="epoch",
            save_strategy="no",
            learning_rate=lr,
            ...
            dataloader_num_workers=0,   # robust (no fork issues)
...
for name, fn, lr, ep, optim_name in CONFIGS:
    per_method_csv = CSV_DIR / f"{safe_method_filename(name)}.csv"

    if SKIP_IF_EXISTS and per_method_csv.exists():
        print(f"\n=== {name} ===")
        print(f"Skipping (already exists): {per_method_csv}")
        continue

    print("\n===", name, "===")
    row = run_experiment(name, fn, lr=lr, epochs=ep, optim_name=optim_name)

    # write immediately so the next method only starts after CSV exists
    write_single_row_csv(row, per_method_csv)
    if WRITE_ROLLUP:
        append_rollup_csv(row, ROLLUP_PATH)

    print("Saved:", per_method_csv)
    if WRITE_ROLLUP:
        print("Updated:", ROLLUP_PATH)

print("\nDone. Per-method CSVs in:", CSV_DIR)