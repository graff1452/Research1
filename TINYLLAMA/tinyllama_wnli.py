...
def make_adapters():
    if not ADAPTERS_IMPORT_OK:
        raise RuntimeError("adapters package not importable.")
    model = build_base_model(False)
    adapters.init(model)
    model.add_adapter("wnli", config="seq_bn")
    model.set_active_adapters("wnli")
    model.train_adapter("wnli")
    return model

def make_lora(r=8, alpha=16, dropout=0.1):
    base = build_base_model(False)
    cfg = LoraConfig(r=r, lora_alpha=alpha, target_modules=["q_proj","v_proj"],
...
out_csv = "tinyllama_wnli_benchmark_results.csv"
