...
print("🚀 BERT SST-2 BitFit Fine-tuning (with checkpoint-averaged power/VRAM)")
print("=" * 60)
...
# BERT doesn't use generation cache; keep for symmetry/robustness
...
# ---------------------------
# BitFit: freeze all; unfreeze biases + LayerNorm + classifier head
# ---------------------------
print("\n🪛 Applying BitFit (bias + LayerNorm + classifier head trainable)...")

for p in model.parameters():
    p.requires_grad = False

# Unfreeze classifier head for BERT seq-classification: 'classifier.*'
...
def is_bitfit_param(n: str) -> bool:
    nl = n.lower()
    # BERT uses LayerNorm.{weight,bias} across encoder layers.
    return (
        nl.endswith(".bias")
        or ".bias" in nl
        or "layernorm.weight" in nl
        or "layernorm.bias" in nl
    )
...
# Use the correct key for your transformers version
# Newer versions: evaluation_strategy; older/forked: eval_strategy
...
# Energy (Wh) = AvgPower(W) * Hours; training_time is in seconds.
energy_Wh = None
