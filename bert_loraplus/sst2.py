...
print("🚀 BERT SST-2 LoRA+ Fine-tuning (with JSON & NVML like sample)")
print("=" * 70)

# --------------------------- Config ---------------------------
MODEL_NAME   = "bert-base-uncased"
OUTPUT_DIR   = "./bert-sst2-loraplus"
BATCH_SIZE   = 32
LEARNING_RATE= 1e-5
EPOCHS       = 3
MAX_LENGTH   = 128
...
# --------------------------- LoRA config ---------------------------
print("\n🔧 Configuring LoRA...")
# BERT attention uses: query/key/value and attention.output.dense (plus other "dense" layers).
# For strict attention-only LoRA, set target_modules=["query","key","value"].
...
args_kwargs = dict(
    ...
    learning_rate=LEARNING_RATE,   # base LR; LoRA+ modifies A vs B internally
    weight_decay=0.01,
    warmup_ratio=0.1,
...
# --------------------------- LoRA+ optimizer ---------------------------
# Prefer PEFT's helper; else fall back to manual param groups.
optimizer = None
scheduler = None
...
# Use HF's internal step estimation when possible; otherwise approximate.
